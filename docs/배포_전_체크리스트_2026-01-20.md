...
- [x] **빌드 결과물**: `dist/` 폴더 생성 확인
- [x] **빌드 크기**: 233.64 KB (정상 범위)
...
- [x] **VPS Gemma3 URL**: `http://148.230.97.246:11434`
- [x] **VPS 표준 API URL**: `http://148.230.97.246:8003` (선택적)
...
**확인 명령어**:
```powershell
curl.exe -s "http://148.230.97.246:11434/api/tags" | ConvertFrom-Json | Select-Object -ExpandProperty models | Where-Object { $_.name -like "*gemma3*" }
```
...
- [x] **타입 체크**: `npm run typecheck` 통과
- [x] **에러 수정**: 13개 타입 에러 모두 수정 완료
...
**절대 금지**:
- ❌ 환경 변수를 수동으로 설정 (자동 설정 사용)
- ❌ 포트 번호를 하드코딩 (3001번 포트 사용)
